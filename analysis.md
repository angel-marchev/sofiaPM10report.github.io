...
$$
\text{CP}_{t} = {sfcWindAVG}_{t}*{sfcWindAVG}_{t - 1}
$$

$$
R_{t} = \frac{{lagP1}_{t}}{{CP}_{t}}
$$

Following the preliminary analysis findings, we define the following dummy variables:

$$
{D1}_{t} = \left\{ \begin{matrix} 1,\ if\ \text{RHMAX}_{t} = 100\% \\0,\ else. \\ \end{matrix} \right.\ 
$$

$$
{D2}_{t} = \left\{ \begin{matrix} 1,\ if\ R\text{sfcWindMIN}_{t} = 0\ km/h \\ 0,\ else. \\ \end{matrix} \right.\ 
$$

$$
{D3}_{t} = \left\{ \begin{matrix} 1,\ if\ \text{PRCPAVG}_{t} = 0\ mm \\0,\ else. \\\end{matrix} \right.\ 
$$
...
Results are benchmarked against two naïve predictive models: $$\left\{ M1:\\text{PM}_{10,\mathbf{T + 1}} = \frac{1}{T}\sum_{t =
1}^{T}\text{PM}_{10,\mathbf{t}} \right\}$$, $$\left\{ M2:\\text{PM}_{10,\mathbf{T + 1}} = \text{PM}_{10,\mathbf{T}} \right\}$$.
