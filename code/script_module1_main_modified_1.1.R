...
#Thresholds
thresholdForRemovalMissingP1 <- 0.2 # Removes all observations where the P1 is equal to 0 for more than the threshold % of observations
thresholdP1ForDeviationFromOfficial <- 1.25 # Caps the P1 of the citizen data at thresholdP1ForDeviationFromOfficial% of official P1 maximum
thresholdForMinimumObservationDays <- 90 # Removes all observations with less than thresholdForMinimumObservationDays days of observation history
thresholdForCloseness <- 0.01 # Determines which stations are considered close enough to comapre the P1 measurments
thresholdForCommonObservations <- 168 # Number of common observations to considered a difference in measurments in P1
thresholdForAbsoluteDifference <- 10 # Threshold above which citizen measurments are considered too different
thresholdForDifferentStations <- 1 # Threshold showing how many close stations can have different measurments before exclusion
